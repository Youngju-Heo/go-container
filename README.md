...
- **크래시 복구** — append-only + 청크별 CRC-32C. 전원 손실 시 별도 도구 없이 마지막 유효 청크까지 그대로 열린다. `Repair`로 미완결 파일을 제자리에서 정상(Footer 있는) 파일로 재마감할 수 있다(무손실·멱등, 이후 오픈이 스캔 없이 빨라진다).
...
// 크래시로 미완결된 파일 복구 (제자리 재마감, 멱등)
res, _ := gmc.Repair("rec.gmc")                     // Footer 없으면 스캔→재마감, 이미 정상이면 no-op
_ = res.Repaired                                    // 변경 여부. res.Frames/Summaries/Size·트랙 요약 제공
```
...
미완성/크래시 파일은 열 때마다 스캔 비용을 치른다. `gmc.Repair()`는 이 스캔을 한 번 수행해 마지막 유효 청크 뒤 잔여 바이트를 잘라내고 Footer+트레일러를 붙여 정상 파일로 만든다(프레임 데이터 무손실, 재호출 시 no-op). 이후 오픈은 "완성된 파일" 경로로 빨라진다.
